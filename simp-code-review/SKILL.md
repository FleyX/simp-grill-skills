---
name: simp-code-review
description: "Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating PRD/ticket asked for?). Runs both reviews in parallel sub-agents and reports them side by side. Invoke manually — for a finished feature, a PR, or any 'review since X' request."
...
- **Spec** — does the code faithfully implement the originating PRD / spec / ticket?

Both axes run as **parallel sub-agents** so they don't pollute each other's context, then this skill aggregates their findings.

This skill is **never invoked automatically** — not by `/simp-implement` (which does its own lightweight per-ticket review), not at the end of a feature. The user decides when a full review is worth its cost.
...
2. A PRD under `docs/prd/` matching the branch name or feature — use the index at `docs/prd/README.md` to find it.
3. Ticket files under `.scratch/*/issues/` matching the work.
...
Anything in the repo that documents how code should be written — `CODING_STANDARDS.md`, `CONTRIBUTING.md`, lint configs with stated rationale. ADRs in `docs/adr/` also count as standards for the areas they cover.
...
- The path or full contents of the spec.
